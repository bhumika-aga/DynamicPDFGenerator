...
public class PdfControllerTest {
	...
	void testGeneratePdf() throws Exception {
		when(pdfService.generateArray(invoice))
				.thenReturn(new ByteArrayOutputStream().assignBytes(new ObjectMapper().writeValueAsBytes(invoice)));
		...
		Assertions.assertEquals(response.getHeaders().getContentDisposition().getFilename(),
				"invoice" + invoice.getSeller() + "-" + invoice.getBuyer() + ".pdf");
